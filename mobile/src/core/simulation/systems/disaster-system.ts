        ...
        for (const kingdomId of allKingdomIds) {
          ...
          if (Math.random() < 0.02) {
            ...
            if (disasterType < 0.3) {
              ...
              // SECA - Reduz comida
              const foodLoss = Math.floor(kingdom.economy.stock[ResourceType.Food] * 0.4);
          ...
          if (Math.random() < 0.015) {
            ...
            if (positiveEventType < 0.4) {
              ...
              const foodBonus = Math.floor(kingdom.economy.stock[ResourceType.Food] * 0.3) + 50;
              kingdom.economy.stock[ResourceType.Food] += foodBonus;
          ...
          if (Math.random() < socialEventChance) {
            if (kingdom.population.unrest > 0.6) {
              ...
              kingdom.population.unrest = roundTo(clamp(kingdom.population.unrest - happinessGain, 0, 1));
