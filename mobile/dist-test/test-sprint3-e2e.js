...
exports.Sprint3E2ETestRunner = void 0;
...
class Sprint3E2ETestRunner {
    ...
    chatHistories = {};
    mockedLLMReplies = ["Saudações, vizinho.", "Nossos povos prosperarão juntos.", "Que o destino guie nossas nações."];
    ...
    simulateLLMCommand(session, command, payload) {
    ...
    registerAllTestCases() {
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                const initialState = (0, create_initial_state_1.createInitialState)(runner.staticWorldData, "r_hex_101", world_definitions_v1_1.WORLD_DEFINITIONS_V1);
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                const initialState = (0, create_initial_state_1.createInitialState)(runner.staticWorldData, "r_hex_408", world_definitions_v1_1.WORLD_DEFINITIONS_V1);
                const state = await session.bootstrap(initialState);
                const playerKingdom = state.kingdoms["k_player"];
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                await session.bootstrap(initialState);
                session.setPaused(true);
                if (session.getState().meta.paused !== true)
                    return { ok: false, message: "Failed pause" };
                session.setPaused(false);
                if (session.getState().meta.paused !== false)
                    return { ok: false, message: "Failed play" };
                session.setPaused(true);
                if (session.getState().meta.paused !== true)
                    return { ok: false, message: "Failed toggle back to pause" };
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                const snapshot = session.buildSaveSlotSnapshot("auto-1");
                await session.deps.saveRepository.saveToSlot(snapshot);
                const list = await session.deps.saveRepository.listSlots();
                if (list.some((s) => s.slotId === "auto-1")) {
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                await session.deps.saveRepository.saveToSlot(snapshot);
                const list = await session.deps.saveRepository.listSlots();
                const autoSlot = list.find((s) => s.slotId === "auto-1");
                if (autoSlot) {
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                const rebootSession = new game_session_1.GameSession({
                    gameStateRepository: new memory_persistence_1.MemoryGameStateRepository(),
                ...
                const list = await rebootSession.deps.saveRepository.listSlots();
                if (list.some((s) => s.slotId === "auto-1")) {
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                // Reset state
                state.meta.tick = 0;
                const loadedSnapshot = await session.deps.saveRepository.loadFromSlot("auto-1");
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                const initialState = (0, create_initial_state_1.createInitialState)(runner.staticWorldData, "r_hex_101", world_definitions_v1_1.WORLD_DEFINITIONS_V1);
                const state = await session.bootstrap(initialState);
                const snapshot1 = session.buildSaveSlotSnapshot("auto-1");
                snapshot1.state.meta.tick = 10;
                await session.deps.saveRepository.saveToSlot(snapshot1);
                const snapshot2 = session.buildSaveSlotSnapshot("auto-1");
                snapshot2.state.meta.tick = 20;
                await session.deps.saveRepository.saveToSlot(snapshot2);
                const loaded = await session.deps.saveRepository.loadFromSlot("auto-1");
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                if (session.devModeActive !== true)
                    return { ok: false, message: "Failed toggle on" };
                session.devModeActive = false;
                if (session.devModeActive !== false)
                    return { ok: false, message: "Failed toggle off" };
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                for (const kingdomId of Object.keys(state.kingdoms)) {
                    if (kingdomId === "k_nature" || kingdomId === "k_player")
                        continue;
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                session.nextRandom = () => 0; // force success
        ...
        this.register({
            ...
            run: async (runner) => {
                const session = runner.createFreshSession();
                const lastRegionId = world_definitions_v1_1.WORLD_DEFINITIONS_V1[world_definitions_v1_1.WORLD_DEFINITIONS_V1.length - 1].id;
                const initialState = (0, create_initial_state_1.createInitialState)(runner.staticWorldData, lastRegionId, world_definitions_v1_1.WORLD_DEFINITIONS_V1);
                const state = await session.bootstrap(initialState);
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                const initialState = (0, create_initial_state_1.createInitialState)(runner.staticWorldData, regionId, world_definitions_v1_1.WORLD_DEFINITIONS_V1);
                const state = await session.bootstrap(initialState);
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                try {
                    await session.deps.saveRepository.saveToSlot({
                        summary: {
                            slotId: "auto-1",
                            savedAt: Date.now(),
                            tick: 0,
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                for (let i = 0; i < 3; i++) {
                    const snapshot = session.buildSaveSlotSnapshot(`manual-${i + 1}`);
                    await session.deps.saveRepository.saveToSlot(snapshot);
                }
                // Autosave should still be writeable
                const autosaveSnap = session.buildSaveSlotSnapshot("auto-1");
                await session.deps.saveRepository.saveToSlot(autosaveSnap);
                const list = await session.deps.saveRepository.listSlots();
                if (list.some((s) => s.slotId === "auto-1")) {
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                const manualSnap = session.buildSaveSlotSnapshot("manual-1");
                await session.deps.saveRepository.saveToSlot(manualSnap);
                const autoSnap = session.buildSaveSlotSnapshot("auto-1");
                await session.deps.saveRepository.saveToSlot(autoSnap);
                const list = await session.deps.saveRepository.listSlots();
                const hasManual = list.some((s) => s.slotId === "manual-1");
                const hasAuto = list.some((s) => s.slotId === "auto-1");
        ...
        this.register({
            ...
            run: async (runner) => {
                ...
                if (!warRes.ok)
                    return { ok: false, message: "War declaration failed" };
        ...
        this.register({
            ...
            run: async (runner) => {
                const saveRepo = new memory_persistence_1.MemorySaveRepository();
                const session = new game_session_1.GameSession({
                    gameStateRepository: new memory_persistence_1.MemoryGameStateRepository(),
                    saveRepository: saveRepo,
                    staticWorldData: runner.staticWorldData,
                    clock: runner.clock,
                ...
                const recoveredSession = new game_session_1.GameSession({
                    gameStateRepository: new memory_persistence_1.MemoryGameStateRepository(),
                    saveRepository: saveRepo,
                    staticWorldData: runner.staticWorldData,
                    clock: runner.clock,
                ...
                const loaded = await recoveredSession.deps.saveRepository.loadFromSlot("auto-1");
                if (loaded) {
...
exports.Sprint3E2ETestRunner = Sprint3E2ETestRunner;
