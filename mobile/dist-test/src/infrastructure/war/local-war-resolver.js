...
exports.LocalWarResolver = void 0;
...
function getDistance(defs, regionA, regionB) {
...
function relationCooldownUntil(state, leftId, rightId) {
...
function applyRelationCooldown(state, leftId, rightId, until) {
...
function militaryPower(kingdom) {
...
function participantPower(state, participants) {
...
function isAtWar(kingdomId, war) {
    return war.attackers.includes(kingdomId) || war.defenders.includes(kingdomId);
}
function areInActiveWar(state, leftId, rightId) {
    for (const warId of Object.keys(state.wars).sort()) {
        const war = state.wars[warId];
...
function hasActivePeaceTreaty(state, leftId, rightId, now) {
    const kingdom = state.kingdoms[leftId];
    if (!kingdom) {
        return false;
    }
    return kingdom.diplomacy.treaties.some((treaty) => treaty.type === enums_1.TreatyType.Peace &&
...
function addPeaceTreaty(state, leftId, rightId, signedAt) {
    const left = state.kingdoms[leftId];
    const right = state.kingdoms[rightId];
    if (!left || !right) {
        return;
...
function findBorderFrontsMany(state, definitions, attackers, defenders) {
    const fronts = [];
    const defenderRegionSet = new Set();
    for (const d of defenders) {
        for (const rId of Object.keys(state.world.regions)) {
            if (state.world.regions[rId].ownerId === d)
                defenderRegionSet.add(rId);
        }
    }
    for (const a of attackers) {
        ...
        for (const rId of attackerRegions) {
            ...
            for (const nId of def.neighbors) {
                if (defenderRegionSet.has(nId))
                    fronts.push({ regionId: nId, pressureAttackers: 50, pressureDefenders: 50 });
            }
        }
    }
    if (fronts.length > 0) {
...
function reduceArmyForWar(kingdom, intensity, war) {
    let deadThisTick = 0;
...
function applyConquest(state, definitions, winners, losers) {
...
class LocalWarResolver {
    ...
    declareWar(state, attackerId, defenderId) {
        ...
        for (const treaty of defenderObj.diplomacy.treaties) {
            if (treaty.type === enums_1.TreatyType.Alliance) {
                const ally = treaty.parties.find(p => p !== defenderId);
                if (ally && state.kingdoms[ally] && !areInActiveWar(state, ally, attackerId))
                    defenders.add(ally);
            }
            if (treaty.type === enums_1.TreatyType.Vassalage) {
                const overlord = treaty.terms.overlordId;
                const vassal = treaty.terms.vassalId;
                if (vassal === defenderId && overlord !== attackerId && state.kingdoms[overlord])
                    defenders.add(overlord);
                if (overlord === defenderId && vassal !== attackerId && state.kingdoms[vassal])
                    defenders.add(vassal);
            }
        }
        const attackerObj = state.kingdoms[attackerId];
        for (const treaty of attackerObj.diplomacy.treaties) {
            if (treaty.type === enums_1.TreatyType.Vassalage && treaty.terms.overlordId === attackerId) {
                const vassal = treaty.terms.vassalId;
                if (vassal !== defenderId && state.kingdoms[vassal])
                    attackers.add(vassal);
            }
        }
        const warId = (0, identifiers_1.buildWarIdFromSides)(Array.from(attackers), Array.from(defenders), state.meta.tick);
        if (state.wars[warId]) {
            return state;
        }
        ...
        for (const a of attackers) {
            for (const d of defenders) {
                setPairStatus(state, a, d, enums_1.DiplomaticRelation.Hostile);
    ...
    enforcePeace(state, warId) {
        ...
        for (const attackerId of war.attackers) {
            for (const defenderId of war.defenders) {
                setPairStatus(state, attackerId, defenderId, enums_1.DiplomaticRelation.Truce);
    ...
    resolveTick(state, now) {
        ...
        for (const warId of Object.keys(state.wars).sort()) {
            ...
            for (const attackerId of war.attackers) {
                ...
                attacker.diplomacy.warExhaustion = roundTo(clamp(attacker.diplomacy.warExhaustion + 0.003 + intensity * 0.004 + longWarPressure * 0.008, 0, 1));
            ...
            for (const defenderId of war.defenders) {
                ...
                defender.diplomacy.warExhaustion = roundTo(clamp(defender.diplomacy.warExhaustion + 0.0035 + intensity * 0.004 + longWarPressure * 0.009, 0, 1));
...
exports.LocalWarResolver = LocalWarResolver;
