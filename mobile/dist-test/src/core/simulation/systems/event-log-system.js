...
exports.createEventLogSystem = createEventLogSystem;
const SEVERITY_RANK = {
    info: 0,
    warning: 1,
    critical: 2
};
function kingdomName(state, kingdomId) {
    if (!kingdomId) {
        return "Desconhecido";
    }
    return state.kingdoms[kingdomId]?.name ?? kingdomId;
}
function buildGroupKey(event, customKey) {
    if (customKey) {
        return customKey;
    }
...
function describeNpcDecision(event, state) {
...
function describeEvent(event, state, staticData) {
    switch (event.type) {
        ...
        case "war.started": {
            ...
            const defenders = event.payload.defenders;
            let extra = "";
...
function stripCountSuffix(details) {
    return details.replace(/\s\(x\d+\)$/u, "");
}
function createEventLogSystem(maxEntries = 180, dedupeWindowMs = 45_000) {
    return {
        id: "event_log",
        run(context) {
            ...
            for (const event of context.events) {
                ...
                const existingIndex = mergedLog.findIndex((entry) => entry.groupKey === groupKey && context.now - entry.occurredAt <= dedupeWindowMs);
                if (existingIndex >= 0) {
