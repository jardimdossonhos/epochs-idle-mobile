...
exports.createWarSystem = createWarSystem;
const utils_1 = require("./utils");
function createWarSystem(warResolver) {
    return {
        id: "war",
        run(context) {
            const stateBefore = context.nextState;
            let eventSeq = 0;
            const ownersBefore = new Map(Object.keys(stateBefore.world.regions)
                .sort()
                .map((regionId) => [regionId, stateBefore.world.regions[regionId].ownerId]));
            const warScoresBefore = new Map(Object.keys(stateBefore.wars)
                .sort()
                .map((warId) => [warId, stateBefore.wars[warId].warScore]));
            ...
            for (const war of warsAfter) {
                ...
                if (previousScore !== undefined && Math.abs(previousScore) < 45 && Math.abs(war.warScore) >= 45) {
                    context.events.push({
                        ...
                        payload: {
                            warId: war.id,
                            warScore: (0, utils_1.roundTo)(war.warScore)
                        },
