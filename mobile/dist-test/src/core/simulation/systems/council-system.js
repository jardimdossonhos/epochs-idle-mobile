...
exports.updateMinisterExperience = updateMinisterExperience;
exports.generateRoutineAdvice = generateRoutineAdvice;
exports.createCouncilSystem = createCouncilSystem;
const enums_1 = require("../../models/enums");
const utils_1 = require("./utils");
const culture_generator_1 = require("./culture-generator");
...
function generateCandidate(idSeq) {
    const roles = Object.values(enums_1.MinisterRole);
    const personalities = Object.values(enums_1.MinisterPersonality);
    const cultureId = (0, culture_generator_1.getRandomCulture)();
    ...
    if (personality === enums_1.MinisterPersonality.Greedy)
        stats.administration += 3;
    if (personality === enums_1.MinisterPersonality.Militarist)
        stats.martial += 3;
    if (personality === enums_1.MinisterPersonality.Pacifist)
        stats.diplomacy += 3;
    if (personality === enums_1.MinisterPersonality.Progressive)
        stats.learning += 3;
    const baseSalary = skill * 4 + (personality === enums_1.MinisterPersonality.Greedy ? 8 : 0);
    return {
...
function updateMinisterExperience(minister, state, kingdomId, context) {
    ...
    if (kingdom.population.growthRatePerTick > 0.0002)
        experienceGain += 2; // Economia/população crescendo
    if (kingdom.population.unrest < 0.3)
        experienceGain += 1; // Povo feliz
    const averageMorale = kingdom.military.armies.length > 0
        ...
        : 0;
    if (averageMorale > 0.8)
        experienceGain += 1; // Exército motivado
    if (kingdom.religion.cohesion > 0.8)
        experienceGain += 1; // Religião unida
    if (kingdom.economy.corruption < 0.1)
        experienceGain += 1; // Baixa corrupção
    // Bônus baseado no papel específico do ministro
    switch (minister.role) {
        case enums_1.MinisterRole.Steward:
            if (kingdom.economy.stock[enums_1.ResourceType.Gold] > 500)
                experienceGain += 2; // Tesouro saudável
            if (kingdom.population.growthRatePerTick > 0.0005)
                experienceGain += 3; // Crescimento excepcional
            break;
        case enums_1.MinisterRole.Marshal:
            ...
            if (kingdom.military.reserveManpower > 200)
                experienceGain += 1; // Forças bem treinadas
            break;
        case enums_1.MinisterRole.Chancellor:
            // Diplomacia é mais complexa - ganha experiência por alianças ativas
            const activeAlliances = kingdom.diplomacy.treaties.filter(t => t.parties.includes(kingdomId) && t.type === 'alliance').length;
            experienceGain += activeAlliances * 2;
            break;
        case enums_1.MinisterRole.Chaplain:
            if (kingdom.religion.cohesion > 0.9)
                experienceGain += 3; // Fé muito forte
            if (kingdom.population.unrest < 0.2)
                experienceGain += 1; // Ordem social
            break;
    }
    ...
    if (minister.experience >= minister.experienceToNext) {
        ...
        if (minister.stats) {
            ...
            switch (minister.personality) {
                case enums_1.MinisterPersonality.Greedy:
                    minister.stats.administration += bonus;
                    break;
                case enums_1.MinisterPersonality.Militarist:
                    minister.stats.martial += bonus;
                    break;
                case enums_1.MinisterPersonality.Pacifist:
                    minister.stats.diplomacy += bonus;
                    break;
                case enums_1.MinisterPersonality.Progressive:
                    minister.stats.learning += bonus;
                    break;
                case enums_1.MinisterPersonality.Zealous:
                    minister.stats.intrigue += bonus; // Zelosos são mais manipuladores
                    break;
                case enums_1.MinisterPersonality.Cautious:
        ...
        context.events.push({
            id: (0, utils_1.createEventId)({ prefix: "evt_minister_levelup", tick: state.meta.tick, systemId: "council", actorId: minister.id, sequence: Math.floor(Math.random() * 1000) }),
...
function evaluateMinisterLoyalty(minister, state, kingdomId) {
    const kingdom = state.kingdoms[kingdomId];
    if (!kingdom)
        return;
    // Fallback para saves antigos sem salário
    if (minister.salary === undefined) {
        minister.salary = minister.skillLevel * 4 + (minister.personality === enums_1.MinisterPersonality.Greedy ? 8 : 0);
    }
    const expectedSalary = minister.skillLevel * 5 + (minister.personality === enums_1.MinisterPersonality.Greedy ? 12 : 0);
    ...
    switch (minister.personality) {
        case enums_1.MinisterPersonality.Militarist:
            ...
            if (kingdom.economy.budgetPriority.military > 30)
                loyaltyDelta += 0.2;
            break;
        case enums_1.MinisterPersonality.Pacifist:
        ...
        case enums_1.MinisterPersonality.Greedy:
            ...
            if (kingdom.economy.corruption > 0.2)
                loyaltyDelta += 0.2; // O ganancioso ama um reino corrupto
            break;
        case enums_1.MinisterPersonality.Zealous:
    ...
    minister.loyalty = (0, utils_1.roundTo)((0, utils_1.clamp)(minister.loyalty + loyaltyDelta, 0, 100));
}
function generateAdvice(minister, state, kingdomId, staticData, activeAdvice) {
    const kingdom = state.kingdoms[kingdomId];
    const isAtWar = Object.values(state.wars).some(w => w.attackers.includes(kingdomId) || w.defenders.includes(kingdomId));
    let text = "";
    let urgency = "low";
    ...
    if (minister.role === enums_1.MinisterRole.Steward) {
        const food = kingdom.economy.stock[enums_1.ResourceType.Food];
        ...
        if (food < pop / 8000 && kingdom.economy.budgetPriority.economy < 35) {
            ...
            if (minister.personality === enums_1.MinisterPersonality.Greedy)
                text = "Os plebeus morrem de fome, Majestade. Isso é péssimo, pois cadáveres não pagam impostos! Libere verbas agrícolas imediatamente.";
            else
                text = "Senhor, nossos celeiros estão vazios. A desnutrição nas províncias pode causar o colapso do reino. Precisamos de investimentos.";
            options = [
            ...
            options = [
                { id: "opt_1", label: "Aprovar: Reduzir Taxa Base em -10%", actionType: "update_tax", payload: { baseRate: Math.max(0.05, currentTax - 0.1) }, loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Greedy ? -20 : 15 },
                { id: "opt_2", label: "Rejeitar: O povo deve pagar", actionType: "ignore", loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Greedy ? 10 : -15 }
            ];
        }
        else if (kingdom.economy.stock[enums_1.ResourceType.Gold] < 100) {
            if (currentTax >= 0.5) {
    ...
    else if (minister.role === enums_1.MinisterRole.Marshal) {
        if (isAtWar) {
            ...
            if (manpower < 100) {
                ...
                if (minister.personality === enums_1.MinisterPersonality.Pacifist)
                    text = "Nossos filhos morrem nas fronteiras, meu Senhor. Imploro que busque um tratado de paz antes que não reste ninguém para lutar.";
                else
                    text = "As espadas estão desembainhadas! Aumente o orçamento militar e massacraremos essa escória antes do inverno.";
                options = [
                    { id: "opt_1", label: "Aprovar Decreto de Guerra: +35% Orçamento Militar", actionType: "update_budget", payload: { military: 35 }, loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Pacifist ? -20 : 15 },
            ...
            if (minister.personality === enums_1.MinisterPersonality.Militarist)
                text = "Os camponeses no sul estão ousados demais. Dê-me a ordem e minhas guarnições pintarão as ruas de vermelho.";
            else
                text = "Há tensão nas províncias. Devemos reforçar as patrulhas para manter a ordem.";
        }
    }
    else if (minister.role === enums_1.MinisterRole.Chaplain) {
        if (kingdom.religion.cohesion < 0.4) {
            if (kingdom.religion.policy !== enums_1.ReligiousPolicy.Zealous) {
                urgency = "high";
                title = "Heresia Descontrolada";
                if (minister.personality === enums_1.MinisterPersonality.Zealous)
                    text = "A blasfêmia apodrece o nosso império por dentro! Se não ativarmos a Inquisição agora, o castigo divino recairá sobre nós.";
                else
                    text = "A verdadeira fé está enfraquecendo. Precisamos enviar mais missionários ou aumentar a isenção do clero.";
                options = [
                    { id: "opt_1", label: "Aprovar Inquisição (Política Fanática)", actionType: "set_religious_policy", payload: { policy: enums_1.ReligiousPolicy.Zealous }, loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Zealous ? 25 : -10 },
                    ...
                    { id: "opt_3", label: "Rejeitar Apelo (Manter Tolerância)", actionType: "ignore", loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Zealous ? -25 : -5 }
                ];
            }
        }
        else if (kingdom.religion.policy === enums_1.ReligiousPolicy.Zealous && kingdom.religion.cohesion > 0.85) {
            urgency = "low";
            ...
            options = [
                { id: "opt_1", label: "Aprovar: Retornar à Ortodoxia", actionType: "set_religious_policy", payload: { policy: enums_1.ReligiousPolicy.Orthodoxy }, loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Zealous ? -15 : 15 },
                { id: "opt_2", label: "Rejeitar: Manter a Inquisição", actionType: "ignore", loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Zealous ? 15 : -10 }
            ];
        }
    }
    else if (minister.role === enums_1.MinisterRole.Chancellor) {
        let highestRivalry = 0;
        let worstRivalId = null;
        ...
        const isAtWarWithThem = worstRivalId ? Object.values(state.wars).some(w => (w.attackers.includes(kingdomId) && w.defenders.includes(worstRivalId)) ||
            (w.attackers.includes(worstRivalId) && w.defenders.includes(kingdomId))) : false;
        if (highestRivalry > 0.75 && worstRivalId && !isAtWarWithThem) {
            ...
            if (vulnerableRegionId) {
                ...
                const hasFortress = vulnRegion?.buildings?.includes(enums_1.BuildingType.Fortress);
                const hasBarracks = vulnRegion?.buildings?.includes(enums_1.BuildingType.Barracks);
                urgency = "high";
                title = `Ameaça de Invasão: ${rival.name}`;
                if (!hasFortress && kingdom.economy.stock[enums_1.ResourceType.Gold] >= 500) {
                    ...
                    options = [
                        { id: "opt_fort", label: `Aprovar: Erigir Fortaleza em ${vulnRegionName} (-500 Ouro)`, actionType: "build_structure", payload: { regionId: vulnerableRegionId, buildingType: enums_1.BuildingType.Fortress }, loyaltyImpact: 15 },
                        { id: "opt_ign", label: "Ignorar Ameaça", actionType: "ignore", loyaltyImpact: -10 }
                    ];
                }
                else if (!hasBarracks && kingdom.economy.stock[enums_1.ResourceType.Gold] >= 200) {
                    ...
                    options = [
                        { id: "opt_bar", label: `Aprovar: Construir Quartel em ${vulnRegionName} (-200 Ouro)`, actionType: "build_structure", payload: { regionId: vulnerableRegionId, buildingType: enums_1.BuildingType.Barracks }, loyaltyImpact: 10 },
                ...
                if (minister.personality === enums_1.MinisterPersonality.Militarist && options.length > 0) {
                    options.push({
    ...
    else if (minister.role === enums_1.MinisterRole.Scholar) {
        const hasUniversity = state.world.regions[kingdom.capitalRegionId]?.buildings?.includes(enums_1.BuildingType.University);
        if (!hasUniversity && kingdom.economy.stock[enums_1.ResourceType.Gold] >= 600) {
            urgency = "medium";
            ...
            options = [
                { id: "opt_1", label: "Aprovar: Construir Universidade (-400 Ouro)", actionType: "build_structure", payload: { regionId: kingdom.capitalRegionId, buildingType: enums_1.BuildingType.University }, loyaltyImpact: 20 },
    ...
    if (text === "" && minister.salary < expectedSalary && minister.loyalty < 65 && Math.random() > 0.6) {
        return {
            ...
            urgency: minister.personality === enums_1.MinisterPersonality.Greedy ? "high" : "medium",
            issuedAt: state.meta.lastUpdatedAt,
            options: [
                { id: "opt_sal_1", label: "Conceder Aumento (+5 Ouro/ciclo)", actionType: "change_salary", payload: { amount: 5 }, loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Greedy ? 20 : 10 },
                { id: "opt_sal_2", label: "Recusar", actionType: "ignore", loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Greedy ? -30 : -15 }
            ],
            resolved: false,
            isRead: false
        };
...
function generateRoutineAdvice(minister, state, kingdomId) {
    const kingdom = state.kingdoms[kingdomId];
    let text = "";
    ...
    if (minister.role === enums_1.MinisterRole.Steward) {
        ...
        options = [
            { id: "opt_rout_1", label: "Decretar Tolerância Religiosa", actionType: "set_religious_policy", payload: { policy: enums_1.ReligiousPolicy.Tolerant }, loyaltyImpact: minister.personality === enums_1.MinisterPersonality.Zealous ? -15 : 15 },
            { id: "opt_rout_2", label: "Manter a Ortodoxia Estrita", actionType: "set_religious_policy", payload: { policy: enums_1.ReligiousPolicy.Orthodoxy }, loyaltyImpact: 0 }
        ];
    }
    else {
        return null;
...
function createCouncilSystem() {
    return {
        ...
        run(context) {
            ...
            for (const role of Object.keys(currentCouncil)) {
                ...
                if ((state.meta.tick + ministerOffset) % 7 !== 0)
                    continue;
                ...
                if (minister.loyalty < 15) {
                    context.events.push({
                        id: (0, utils_1.createEventId)({ prefix: "evt_council_resign", tick: state.meta.tick, systemId: "council", actorId: player.id, sequence: eventSeq++ }),
                ...
                if (!hasRecentAdvice) {
                    ...
                    if (advice) {
                        ...
                        context.events.push({
                            id: (0, utils_1.createEventId)({ prefix: "evt_council_advice", tick: state.meta.tick, systemId: "council", actorId: player.id, sequence: eventSeq++ }),
