...
exports.generateHeir = generateHeir;
exports.createCharacterSystem = createCharacterSystem;
...
function generateHeir(ruler, kingdomId, currentTick) {
    const cultureId = ruler.cultureId || 'latin';
    const gender = (0, culture_generator_1.getRandomGender)();
    const generatedName = (0, culture_generator_1.generateCulturalName)(cultureId, gender);
    ...
    return {
        ...
        name: fullName,
        cultureId,
        portraitSeed: (0, culture_generator_1.generatePortraitSeed)(),
        gender,
        title: title,
        isLegendary: false,
...
function processSuccession(kingdom, deadRuler, state, context, eventSeq) {
    // Verifica se o reino tem herdeiros
    if (!kingdom.heirs || kingdom.heirs.length === 0) {
        // Sem herdeiros - crise de sucessão!
        context.events.push({
            id: (0, utils_1.createEventId)({ prefix: "evt_succession_crisis", tick: state.meta.tick, systemId: "character", sequence: eventSeq++ }),
    ...
    if (newRuler) {
        ...
        if (kingdom.npc) {
            const sovereignTraitId = newRuler.traits.find((t) => t !== "nobre" && t !== "herdeiro");
        ...
        context.events.push({
            id: (0, utils_1.createEventId)({ prefix: "evt_succession", tick: state.meta.tick, systemId: "character", sequence: eventSeq++ }),
            ...
            for (const charId in state.world.characters) {
                ...
                if (age >= deathThreshold) {
                    ...
                    if (Math.random() < deathChance) {
                        ...
                        context.events.push({
                            id: (0, utils_1.createEventId)({ prefix: "evt_char_death", tick: state.meta.tick, systemId: "character", sequence: eventSeq++ }),
