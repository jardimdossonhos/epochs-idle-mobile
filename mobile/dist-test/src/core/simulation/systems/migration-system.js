...
exports.createMigrationSystem = createMigrationSystem;
...
function createMigrationSystem(staticData, orderedDefinitions) {
    ...
    return {
        ...
        run: (context) => {
            ...
            for (let offset = 0; offset < orderedDefinitions.length; offset++) {
                ...
                if (!region || region.ownerId === "k_nature")
                    continue;
                ...
                if (currentPop < 15 && kingdom.capitalRegionId !== regionId) {
                    ...
                    if (state.ecs) {
                        ...
                        if (state.ecs.food)
                            state.ecs.food[i] = 0;
                        if (state.ecs.populationTotal)
                            state.ecs.populationTotal[i] = 0;
                        if (state.ecs.manpower)
                            state.ecs.manpower[i] = 0;
                    }
                    context.events.push({
                        id: (0, utils_1.createEventId)({ prefix: "evt_ext", tick: state.meta.tick, systemId: "migration", actorId: kingdom.id, sequence: eventSeq++ }),
                ...
                if (kingdom.administration.automation.expansion === enums_1.AutomationLevel.Manual) {
