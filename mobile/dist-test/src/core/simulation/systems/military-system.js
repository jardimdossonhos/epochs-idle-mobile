...
exports.createMilitarySystem = createMilitarySystem;
function createMilitarySystem(orderedDefinitions) {
    return {
        id: "military_system",
        run: (context) => {
            ...
            for (const kingdomId in state.kingdoms) {
                ...
                for (const army of kingdom.military.armies) {
                    ...
                    if (army.supply < 1.0)
                        army.supply = Math.min(1.0, army.supply + 0.03);
                }
