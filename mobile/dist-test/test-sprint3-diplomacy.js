...
const mockAsyncStorage = {
    ...
    clear: async () => { for (const key in mockStorage)
...
const gemini_service_1 = require("./src/application/ai/gemini-service");
...
async function runTests() {
    ...
    await gemini_service_1.geminiService.setAiEnabled(false);
    const resultFallback = await gemini_service_1.geminiService.chatWithSovereign("Rei Arthur", "Soberano", "latin", ["charismatic"], { administration: 12, martial: 10, diplomacy: 15, intrigue: 8, learning: 11 }, { greed: 0.2, honor: 0.9, caution: 0.5, zeal: 0.6, ambition: 0.4, betrayalTendency: 0.1 }, { status: enums_1.DiplomaticRelation.Neutral, score: { trust: 0.5, fear: 0.2, rivalry: 0.2 } }, "Saudações", []);
    ...
    const state = await session.bootstrap((0, create_initial_state_1.createInitialState)(staticWorldData, undefined, world_definitions_v1_1.WORLD_DEFINITIONS_V1));
    ...
    gemini_service_1.geminiService.chatWithSovereign = async () => {
        return {
    ...
    if (relationAfter.status !== enums_1.DiplomaticRelation.Hostile) {
    ...
    const warInvolvingBoth = activeWars.some(w => (w.attackers.includes("k_player") && w.defenders.includes(npcKingdomId)) ||
