...
import { MemoryGameStateRepository, MemorySaveRepository } from './src/ui/memory-persistence';
import { AutomationLevel } from './src/core/models/enums';
...
function getRegionCounts(state: GameState): Record<string, number> {
...
function logMilestone(year: number, state: GameState) {
  console.log(`\n=================== MILESTONE: Year ${year} ===================`);
  ...
  for (const kingdomId of Object.keys(state.kingdoms).sort()) {
    ...
    const ruler = kingdom.rulerId ? (characters[kingdom.rulerId] as any) : undefined;
  ...
  try {
    ...
    for (const kingdomId of Object.keys(initialState.kingdoms)) {
      ...
      if (kingdom.administration && kingdom.administration.automation) {
        kingdom.administration.automation.expansion = AutomationLevel.Assisted;
        kingdom.administration.automation.economy = AutomationLevel.NearlyAutomatic;
        kingdom.administration.automation.construction = AutomationLevel.NearlyAutomatic;
        kingdom.administration.automation.defense = AutomationLevel.NearlyAutomatic;
        kingdom.administration.automation.technology = AutomationLevel.NearlyAutomatic;
        kingdom.administration.automation.diplomacyReactive = AutomationLevel.NearlyAutomatic;
      }
    }
    ...
    const techUnlocks: Array<{ tick: number; year: number; kingdomId: string; techId: string; name: string }> = [];
    let totalDeaths = 0;
    ...
    let rel1to2AtYear2: any = null;
    let rel2to1AtYear2: any = null;
    ...
    eventBus.subscribe("technology.completed", (event) => {
      ...
      techUnlocks.push({
        ...
        techId: event.payload.technologyId as string,
        name: event.payload.technologyName as string
      });
    ...
    for (let batch = 1; batch <= 20; batch++) {
      ...
      const tickDurationMs = 3000;
      let currentState = newSession.getState();
      
      if (batch === 1) {
        // Run first 25 ticks to capture Year 2 stats
        const startNow = clock.now() + currentState.meta.tick * tickDurationMs;
        const res25 = (newSession as any).pipeline.runBatch(currentState, 25, tickDurationMs, startNow, {
          collectEvents: true,
          maxCollectedEvents: 10000
        });
        (newSession as any).currentState = res25.state;
        for (const event of res25.events) {
          eventBus.publish(event);
        }
        
        // Capture Year 2 stats
        ...
        // Run remaining 1175 ticks of Batch 1
        currentState = newSession.getState();
        const startNow2 = clock.now() + currentState.meta.tick * tickDurationMs;
        const resRemainder = (newSession as any).pipeline.runBatch(currentState, BATCH_SIZE - 25, tickDurationMs, startNow2, {
          collectEvents: true,
          maxCollectedEvents: 10000
        });
        (newSession as any).currentState = resRemainder.state;
        for (const event of resRemainder.events) {
          eventBus.publish(event);
        }
      } else {
        // Run full BATCH_SIZE (1200 ticks) using runBatch
        const startNow = clock.now() + currentState.meta.tick * tickDurationMs;
        const resBatch = (newSession as any).pipeline.runBatch(currentState, BATCH_SIZE, tickDurationMs, startNow, {
          collectEvents: true,
          maxCollectedEvents: 10000
        });
        (newSession as any).currentState = resBatch.state;
        for (const event of resBatch.events) {
          eventBus.publish(event);
        }
      }
    ...
    for (const kingdomId of Object.keys(finalState.kingdoms).sort()) {
      ...
      const ruler = kingdom.rulerId ? (finalCharacters[kingdom.rulerId] as any) : undefined;
      ...
      if (kingdom.heirs) {
        for (const heirId of kingdom.heirs) {
          const heir = heirId ? (finalCharacters[heirId] as any) : undefined;
          if (heir) {
